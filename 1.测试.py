...
# 打开BDF文件
mat_file_name = r'E:\2大学项目\DEAP\data_preprocessed_matlab\s01.mat'
to_save_GSR = r'E:\2大学项目\my预处理\s01_GSR.xlsx'
to_save_PPG = r'E:\2大学项目\my预处理\s01_PPG.xlsx'
...
print("cA3",cA3.shape)
print("cA3_denoised",cA3_denoised.shape)
# 更新系数
coeffs_denoised = [cA3_denoised, cD3, cD2, cD1]
print("coeffs",len(coeffs[0][0]))
print("coeffs_denoised",coeffs_denoised[0][0])
# 使用去噪后的系数重构信号
reconstructed_signal = pywt.waverec(coeffs_denoised, wavelet)
#这是原始信号
raw_signal = pywt.waverec(coeffs, wavelet)
print("reconstructed_signal",reconstructed_signal[0])
print("raw_signal",raw_signal[0])
for i in range(40):
  print("$$$$",reconstructed_signal[0][i]-raw_signal[0][i])#计算信号差值
...
# # 原始信号
# plt.subplot(2, 1, 1)
# plt.plot(np.arange(len(raw_PPG)) / 128, raw_PPG, label='原始信号')
# plt.xlabel('时间 (秒)')
# plt.ylabel('PPG 信号')
# plt.title('原始 PPG 信号')
# plt.legend()
...
# # 过滤后的信号
# plt.subplot(2, 1, 2)
# plt.plot(t, reconstructed_signal[0], label='过滤后的信号', color='r')
# plt.xlabel('时间 (秒)')
# plt.ylabel('PPG 信号')
# plt.title('过滤后的 PPG 信号')
# plt.legend()
#
# plt.tight_layout()
# plt.show()
