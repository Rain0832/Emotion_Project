...
def combine_feauture_and_labels():
    gsr_feature = pd.read_excel(r"E:\2大学项目\2重新写特征\GSR3个特征（带行列索引）.xlsx",
                                index_col=0, usecols=range(0, 4), nrows=None)
    ...
    labels = pd.read_excel(r"E:\2大学项目\2重新写特征\二分类原始标签（带索引）.xlsx", index_col=0)
    #先合并gsr+ppg
    # gsr_ppg = pd.concat([gsr_feature, ppg_feature], axis=1)
    # gsr_ppg.columns = range(1, len(gsr_ppg.columns) + 1)
    #再合并label
    result = pd.concat([gsr_feature, labels], axis=1)
    #存为excel
    resultPath1 = r"E:\2大学项目\2重新写特征\2重新写特征\所有原始特征+标签3（带行列索引）.xlsx"  # 指定excel的路径,
    df1 = pd.DataFrame(result)
    df1.to_excel(resultPath1, sheet_name="所有原始特征+标签3（带行列索引）")
    return 1

#筛选所有
def select_feauture_and_labels():
    previous_data = pd.read_excel(r"E:\2大学项目\2重新写特征\2重新写特征\所有原始特征+标签3（带行列索引）.xlsx",
    ...
    resultPath2 = r"E:\2大学项目\2重新写特征\2重新写特征\筛选后特征+标签3（带行列索引）.xlsx"  # 指定excel的路径,
    df2 = pd.DataFrame(selected_data)
    df2.reset_index(drop=True, inplace=True)  # 重置行索引，丢弃旧的索引
    df2.to_excel(resultPath2, sheet_name="筛选后特征+标签3（带行列索引）")
    return 1
