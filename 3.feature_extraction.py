...
def gsr_features_to_excel():
    ##gsr
    gsr_features = []
    gsr_total = pd.read_excel(r"E:\2大学项目\final_project_for_memory\1.preprocessed_GSR_final.xlsx",
    ...
    resultPath1 = r"E:\2大学项目\final_project_for_memory\3.GSR4个特征（带行列索引）.xlsx"  # 指定excel的路径,
...
def ppg_features_to_excel():
    ##ppg
    ppg_features = []
    ppg_total = pd.read_excel(r"E:\2大学项目\final_project_for_memory\2.ppg归一化数据_最小最大值（带行列索引）.xlsx",
                              index_col=0,header=0)
    ##usecols=range(0,8065)表示读0到8064列，index_col=0表示第0列作为行索引，nrows=4读前4行
    ##读数据时，即使打印出索引，真正读时也不会将索引当成真正数据，因此原数据最好保留索引
    ##DataFrame形式
    ...
    resultPath3 = r"E:\2大学项目\final_project_for_memory\3.PPG1个特征（带行列索引）.xlsx"  # 指定excel的路径,
